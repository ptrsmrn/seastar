...
//
// Exercises chardev_data_source_impl / chardev_data_sink_impl using an
// anonymous OS pipe obtained from reactor::make_pipe().  The write end is
// wrapped in make_chardev_output_stream() and the read end in
// make_chardev_input_stream().
// ---------------------------------------------------------------------------
SEASTAR_THREAD_TEST_CASE(pipe_stream_roundtrip) {
    auto [read_fd, write_fd] = make_pipe().get();

    auto in  = make_chardev_input_stream (std::move(read_fd));
    auto out = make_chardev_output_stream(std::move(write_fd));
...
SEASTAR_THREAD_TEST_CASE(chardev_pty_stream_roundtrip) {
    ...
    auto out = make_chardev_output_stream(std::move(slave));
    auto in  = make_chardev_input_stream (std::move(master));
